...
class HtmlAttributeName implements Comparable<Object> {
  ...
  const HtmlAttributeName(this.prefix, this.name, this.namespace);
  ...
  int compareTo(Object other) {
    ...
    if (other is! HtmlAttributeName) return 1;
...
mixin _ParentNode implements HtmlNode {
  ...
  /// and *-of-type is not implemented. If a selector is not implemented this
  /// method will throw [UnimplementedError].
  List<HtmlElement> querySelectorAll(String selector) =>
...
mixin _NonElementParentNode implements _ParentNode {
  ...
  HtmlElement? getElementById(String id) => querySelector('#$id');
}
...
mixin _HtmlElementAndDocument implements _ParentNode {
  // TODO(jmesserly): could be faster, should throw on invalid tag/class names.

  List<HtmlElement> getElementsByTagName(String localName) =>
      querySelectorAll(localName);

  List<HtmlElement> getElementsByClassName(String classNames) =>
      querySelectorAll(classNames.splitMapJoin(' ',
          onNonMatch: (m) => m.isNotEmpty ? '.$m' : m, onMatch: (m) => ''));
}

/// Really basic implementation of a DOM-core like HtmlNode.
abstract class HtmlNode {
  ...
  /// not an element.
  HtmlElement? get parent {
    final parentNode = this.parentNode;
    return parentNode is HtmlElement ? parentNode : null;
  }

  // TODO(jmesserly): should move to HtmlElement.
  ...
  /// a LinkedHashMap. Each key is a [String] or [HtmlAttributeName].
  ...
  late final List<HtmlElement> children = FilteredElementList(this);
  ...
  void append(HtmlNode node) => nodes.add(node);

  HtmlNode? get firstChild => nodes.isNotEmpty ? nodes[0] : null;
  ...
  void insertBefore(HtmlNode node, HtmlNode? refNode) {
    if (refNode == null) {
      nodes.add(node);
    } else {
  ...
  HtmlNode replaceWith(HtmlNode otherNode) {
    if (parentNode == null) {
      throw UnsupportedError('HtmlNode must have a parent to replace it.');
    }
  ...
  void reparentChildren(HtmlNode newParent) {
  ...
  bool contains(HtmlNode node) => nodes.contains(node);
  ...
  T _clone<T extends HtmlNode>(T shallowClone, bool deep) {
    if (deep) {
      for (var child in nodes) {
...
class HtmlDocument extends HtmlNode
    with _ParentNode, _NonElementParentNode, _HtmlElementAndDocument {
  HtmlDocument() : super._();

  factory HtmlDocument.html(String html) => parse(html);

  @override
  int get nodeType => HtmlNode.DOCUMENT_NODE;

  // TODO(jmesserly): optmize this if needed
  HtmlElement? get documentElement => querySelector('html');

  HtmlElement? get head => documentElement?.querySelector('head');

  HtmlElement? get body => documentElement?.querySelector('body');
  ...
  // only HtmlElement has outerHtml. However it is quite useful. Should we move it
  ...
  @override
  HtmlDocument clone(bool deep) => _clone(HtmlDocument(), deep);

  HtmlElement createElement(String tag) => HtmlElement.tag(tag);

  // TODO(jmesserly): this is only a partial implementation of:
  // http://dom.spec.whatwg.org/#dom-document-createelementns
  HtmlElement createElementNS(String? namespaceUri, String? tag) {
    if (namespaceUri == '') namespaceUri = null;
    return HtmlElement._(tag, namespaceUri);
  }

  DocumentFragment createDocumentFragment() => DocumentFragment();
}

class DocumentFragment extends HtmlNode with _ParentNode, _NonElementParentNode {
  DocumentFragment() : super._();

  factory DocumentFragment.html(String html) => parseFragment(html);

  @override
  int get nodeType => HtmlNode.DOCUMENT_FRAGMENT_NODE;
...
class DocumentType extends HtmlNode {
  final String? name;
  ...
  @override
  int get nodeType => HtmlNode.DOCUMENT_TYPE_NODE;
...
class Text extends HtmlNode {
  ...
  @override
  int get nodeType => HtmlNode.TEXT_NODE;
  ...
  @override
  // Text has a non-nullable `text` field, while HtmlNode has a nullable field.
...
class HtmlElement extends HtmlNode with _ParentNode, _HtmlElementAndDocument {
  ...
  HtmlElement._(this.localName, [this.namespaceUri]) : super._();

  HtmlElement.tag(this.localName)
  ...
  factory HtmlElement.html(String html) {
  ...
  @override
  int get nodeType => HtmlNode.ELEMENT_NODE;

  // TODO(jmesserly): we can make this faster
  HtmlElement? get previousElementSibling {
    if (parentNode == null) return null;
    final siblings = parentNode!.nodes;
    for (var i = siblings.indexOf(this) - 1; i >= 0; i--) {
      final s = siblings[i];
      if (s is HtmlElement) return s;
    }
    return null;
  }

  HtmlElement? get nextElementSibling {
    final parentNode = this.parentNode;
    if (parentNode == null) return null;
    final siblings = parentNode.nodes;
  ...
  // <https://api.dartlang.org/apidocs/channels/stable/#dart-dom-html.HtmlElement@id_setInnerHtml>
  ...
  void _addOuterHtml(StringBuffer str) {
    // http://www.whatwg.org/specs/web-apps/current-work/multipage/the-end.html#serializing-html-fragments
    // HtmlElement is the most complicated one.
    str.write('<');
    ...
    if (attributes.isNotEmpty) {
      attributes.forEach((key, v) {
        // Note: HtmlAttributeName.toString handles serialization of attribute
        // namespace, if needed.
        str.write(' ');
        str.write(key);
        str.write('="');
  ...
  @override
  HtmlElement clone(bool deep) {
    final result = HtmlElement._(localName, namespaceUri)
...
class Comment extends HtmlNode {
  ...
  @override
  int get nodeType => HtmlNode.COMMENT_NODE;
...
class NodeList extends ListProxy<HtmlNode> {
  final HtmlNode _parent;

  NodeList._(this._parent);

  HtmlNode _setParent(HtmlNode node) {
  ...
  void addLast(HtmlNode value) => add(value);

  @override
  void addAll(Iterable<HtmlNode> iterable) {
  ...
  @override
  void insert(int index, HtmlNode element) {
    if (element is DocumentFragment) {
  ...
  @override
  HtmlNode removeLast() => super.removeLast()..parentNode = null;

  @override
  HtmlNode removeAt(int index) => super.removeAt(index)..parentNode = null;
  ...
  @override
  void operator []=(int index, HtmlNode value) {
  ...
  @override
  void setRange(int start, int end, Iterable<HtmlNode> iterable,
      [int skipCount = 0]) {
    var fromVar = iterable as List<HtmlNode>;
  ...
  @override
  void replaceRange(int start, int end, Iterable<HtmlNode> newContents) {
  ...
  @override
  void removeWhere(bool Function(HtmlNode) test) {
  ...
  @override
  void retainWhere(bool Function(HtmlNode) test) {
  ...
  @override
  void insertAll(int index, Iterable<HtmlNode> iterable) {
  ...
  List<HtmlNode> _flattenDocFragments(Iterable<HtmlNode> collection) {
...
// TODO(jmesserly): "implements List<HtmlElement>" is a workaround for analyzer bug.
class FilteredElementList extends IterableBase<HtmlElement>
    with ListMixin<HtmlElement>
    implements List<HtmlElement> {
  final List<HtmlNode> _childNodes;
  ...
  FilteredElementList(HtmlNode node) : _childNodes = node.nodes;
  ...
  List<HtmlElement> get _filtered =>
      _childNodes.whereType<HtmlElement>().toList(growable: false);

  @override
  void forEach(void Function(HtmlElement) action) {
  ...
  @override
  void addAll(Iterable<HtmlElement> iterable) {
  ...
  bool contains(Object? element) {
    return element is HtmlElement && _childNodes.contains(element);
  }

  @override
  Iterable<HtmlElement> get reversed => _filtered.reversed;

  @override
  void sort([int Function(HtmlElement, HtmlElement)? compare]) {
    throw UnsupportedError('TODO(jacobr): should we impl?');
  }

  @override
  void setRange(int start, int end, Iterable<HtmlElement> iterable,
      [int skipCount = 0]) {
    throw UnimplementedError();
  }

  @override
  void fillRange(int start, int end, [HtmlElement? fill]) {
    throw UnimplementedError();
  }

  @override
  void replaceRange(int start, int end, Iterable<HtmlElement> newContents) {
    throw UnimplementedError();
  }
  ...
  @override
  HtmlElement removeLast() {
    return last..remove();
  }

  @override
  Iterable<T> map<T>(T Function(HtmlElement) f) => _filtered.map(f);

  @override
  Iterable<HtmlElement> where(bool Function(HtmlElement) test) => _filtered.where(test);

  @override
  Iterable<T> expand<T>(Iterable<T> Function(HtmlElement) f) => _filtered.expand(f);

  @override
  void insert(int index, HtmlElement element) {
    _childNodes.insert(index, element);
  }

  @override
  void insertAll(int index, Iterable<HtmlElement> iterable) {
    _childNodes.insertAll(index, iterable);
  }

  @override
  HtmlElement removeAt(int index) {
  ...
  bool remove(Object? element) {
    if (element is! HtmlElement) return false;
  ...
  @override
  HtmlElement reduce(HtmlElement Function(HtmlElement, HtmlElement) combine) {
    return _filtered.reduce(combine);
  }

  @override
  T fold<T>(
      T initialValue, T Function(T previousValue, HtmlElement element) combine) {
    return _filtered.fold(initialValue, combine);
  }

  @override
  bool every(bool Function(HtmlElement) test) => _filtered.every(test);

  @override
  bool any(bool Function(HtmlElement) test) => _filtered.any(test);

  @override
  List<HtmlElement> toList({bool growable = true}) =>
      List<HtmlElement>.of(this, growable: growable);

  @override
  Set<HtmlElement> toSet() => Set<HtmlElement>.from(this);

  @override
  HtmlElement firstWhere(bool Function(HtmlElement) test,
      {HtmlElement Function()? orElse}) {
    return _filtered.firstWhere(test, orElse: orElse);
  }

  @override
  HtmlElement lastWhere(bool Function(HtmlElement) test, {HtmlElement Function()? orElse}) {
    return _filtered.lastWhere(test, orElse: orElse);
  }

  @override
  HtmlElement singleWhere(bool Function(HtmlElement) test,
      {HtmlElement Function()? orElse}) {
  ...
  @override
  HtmlElement elementAt(int index) {
    return this[index];
  }
  ...
  @override
  HtmlElement operator [](int index) => _filtered[index];

  @override
  Iterator<HtmlElement> get iterator => _filtered.iterator;

  @override
  List<HtmlElement> sublist(int start, [int? end]) => _filtered.sublist(start, end);

  @override
  Iterable<HtmlElement> getRange(int start, int end) =>
      _filtered.getRange(start, end);

  @override
  int indexOf(Object? element, [int start = 0]) =>
      // Cast forced by ListMixin https://github.com/dart-lang/sdk/issues/31311
      _filtered.indexOf(element as HtmlElement, start);

  @override
  int lastIndexOf(Object? element, [int? start]) {
    start ??= length - 1;
    // Cast forced by ListMixin https://github.com/dart-lang/sdk/issues/31311
    return _filtered.lastIndexOf(element as HtmlElement, start);
  }

  @override
  HtmlElement get first => _filtered.first;

  @override
  HtmlElement get last => _filtered.last;

  @override
  HtmlElement get single => _filtered.single;
}

// http://dom.spec.whatwg.org/#dom-node-textcontent
// For HtmlElement and DocumentFragment
String _getText(HtmlNode node) => (_ConcatTextVisitor()..visit(node)).toString();

void _setText(HtmlNode node, String? value) {
  node.nodes.clear();
