...
/// [HtmlNode.sourceSpan] property will be `null`. When using [generateSpans] you
/// can additionally pass [sourceUrl] to indicate where the [input] was
/// extracted from.
HtmlDocument parse(dynamic input,
...
///
/// Set [generateSpans] if you want to generate [SourceSpan]s, otherwise the
/// [HtmlNode.sourceSpan] property will be `null`. When using [generateSpans] you can
...
class HtmlParser extends IHtmlParser {
  /// Raise an exception on the first error encountered.
  final bool strict;

  /// True to generate [SourceSpan]s for the [HtmlNode.sourceSpan] property.
  ...
  /// DOM will do if you request `HtmlElement.outerHTML`, for example.
  ...
  bool isHTMLIntegrationPoint(HtmlElement element) {
  ...
  bool isMathMLTextIntegrationPoint(HtmlElement element) {
  ...
  void adjustForeignAttributes(StartTagToken token) {
    ...
    // the HtmlNode.attributes Map. Is there another solution?
    final replacements = const {
      'xlink:actuate': HtmlAttributeName('xlink', 'actuate', Namespaces.xlink),
      'xlink:arcrole': HtmlAttributeName('xlink', 'arcrole', Namespaces.xlink),
      'xlink:href': HtmlAttributeName('xlink', 'href', Namespaces.xlink),
      'xlink:role': HtmlAttributeName('xlink', 'role', Namespaces.xlink),
      'xlink:show': HtmlAttributeName('xlink', 'show', Namespaces.xlink),
      'xlink:title': HtmlAttributeName('xlink', 'title', Namespaces.xlink),
      'xlink:type': HtmlAttributeName('xlink', 'type', Namespaces.xlink),
      'xml:base': HtmlAttributeName('xml', 'base', Namespaces.xml),
      'xml:lang': HtmlAttributeName('xml', 'lang', Namespaces.xml),
      'xml:space': HtmlAttributeName('xml', 'space', Namespaces.xml),
      'xmlns': HtmlAttributeName(null, 'xmlns', Namespaces.xmlns),
      'xmlns:xlink': HtmlAttributeName('xmlns', 'xlink', Namespaces.xmlns)
    };
...
class AfterHeadPhase extends Phase {
  ...
  void startTagFromHead(StartTagToken token) {
    parser.parseError(token.span, 'unexpected-start-tag-out-of-my-head',
        {'name': token.name});
    tree.openElements.add(tree.headPointer as HtmlElement);
...
class InBodyPhase extends Phase {
  ...
  bool isMatchingFormattingElement(HtmlElement node1, HtmlElement node2) {
  ...
  void addFormattingElement(StartTagToken token) {
    ...
    final matchingElements = <HtmlNode?>[];
    for (HtmlNode? node in tree.activeFormattingElements.reversed) {
      if (node == null) {
        break;
      } else if (isMatchingFormattingElement(node as HtmlElement, element)) {
  ...
  void endTagFormatting(EndTagToken token) {
    ...
    while (outerLoopCounter < 8) {
      ...
      while (innerLoopCounter < 3) {
        ...
        // HtmlNode is element before node in open elements
        index -= 1;
...
Pair<String, String?> getElementNameTuple(HtmlElement e) {
