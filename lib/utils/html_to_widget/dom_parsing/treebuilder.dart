...
class ActiveFormattingElements extends ListProxy<HtmlElement?> {
...
bool _nodesEqual(HtmlElement node1, HtmlElement node2) {
...
class TreeBuilder extends ITreeBuilder {
  final String? defaultNamespace;

  late HtmlDocument document;

  final List<HtmlElement> openElements = <HtmlElement>[];

  final activeFormattingElements = ActiveFormattingElements();

  HtmlNode? headPointer;

  HtmlElement? formPointer;
  ...
  bool elementInScope(dynamic target, {String? variant}) {
    ...
    // with that name.
    final exactNode = target is HtmlNode;
  ...
  /// return null.
  HtmlElement? elementInActiveFormattingElements(String? name) {
  ...
  void insertComment(StringToken token, [HtmlNode? parent]) {
    parent ??= openElements.last;
    parent.nodes.add(Comment(token.data)..sourceSpan = token.span);
  }

  /// Create an element but don't insert it anywhere
  HtmlElement createElement(StartTagToken token) {
  ...
  HtmlElement insertElement(StartTagToken token) {
    if (insertFromTable) return insertElementTable(token);
    return insertElementNormal(token);
  }

  HtmlElement insertElementNormal(StartTagToken token) {
    final name = token.name;
    final namespace = token.namespace ?? defaultNamespace;
    final element = document.createElementNS(namespace, name)
      ..attributes = token.data
  ...
  HtmlElement insertElementTable(StartTagToken token) {
  ...
  void insertText(String data, FileSpan? span) {
    ...
    if (!insertFromTable ||
      ...
      // special magic element rearranging
      final nodePos = getTableMisnestedNodePosition();
      _insertText(nodePos[0]!, data, span, nodePos[1] as HtmlElement?);
    }
  }

  /// Insert [data] as text in the current node, positioned before the
  /// start of node [refNode] or to the end of the node's text.
  static void _insertText(HtmlNode parent, String data, FileSpan? span,
      [HtmlElement? refNode]) {
  ...
  List<HtmlNode?> getTableMisnestedNodePosition() {
    // The foster parent element is the one which comes before the most
    // recently opened table element
    // XXX - this is really inelegant
    HtmlElement? lastTable;
    HtmlNode? fosterParent;
