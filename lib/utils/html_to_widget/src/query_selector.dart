...
HtmlElement? querySelector(HtmlNode node, String selector) =>
    SelectorEvaluator().querySelector(node, _parseSelectorList(selector));

List<HtmlElement> querySelectorAll(HtmlNode node, String selector) {
  final results = <HtmlElement>[];
  SelectorEvaluator()
...
class SelectorEvaluator extends Visitor {
  ...
  bool matches(HtmlElement element, SelectorGroup selector) {
    _element = element;
    return visitSelectorGroup(selector);
  }

  HtmlElement? querySelector(HtmlNode root, SelectorGroup selector) {
    for (var element in root.nodes.whereType<HtmlElement>()) {
  ...
  void querySelectorAll(
      HtmlNode root, SelectorGroup selector, List<HtmlElement> results) {
    for (var element in root.nodes.whereType<HtmlElement>()) {
    ...
    switch (node.name) {
      ...
      case 'empty':
        return _element!.nodes
            .any((n) => !(n is HtmlElement || n is Text && n.text.isNotEmpty));

      // http://dev.w3.org/csswg/selectors-4/#the-blank-pseudo
      case 'blank':
        return _element!.nodes.any((n) => !(n is HtmlElement ||
  ...
  static String? _getInheritedLanguage(HtmlNode? node) {
    while (node != null) {
