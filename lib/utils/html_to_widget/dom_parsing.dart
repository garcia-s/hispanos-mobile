...
class TreeVisitor {
  void visit(HtmlNode node) {
    switch (node.nodeType) {
      case HtmlNode.ELEMENT_NODE:
        return visitElement(node as HtmlElement);
      case HtmlNode.TEXT_NODE:
        return visitText(node as Text);
      case HtmlNode.COMMENT_NODE:
        return visitComment(node as Comment);
      case HtmlNode.DOCUMENT_FRAGMENT_NODE:
        return visitDocumentFragment(node as DocumentFragment);
      case HtmlNode.DOCUMENT_NODE:
        return visitDocument(node as HtmlDocument);
      case HtmlNode.DOCUMENT_TYPE_NODE:
  ...
  void visitChildren(HtmlNode node) {
  ...
  /// call [visit] instead.
  void visitNodeFallback(HtmlNode node) => visitChildren(node);

  void visitDocument(HtmlDocument node) => visitNodeFallback(node);
  ...
  void visitElement(HtmlElement node) => visitNodeFallback(node);
...
String htmlToCodeMarkup(HtmlNode node) {
...
class CodeMarkupVisitor extends TreeVisitor {
  ...
  @override
  void visitDocument(HtmlDocument node) {
    _str.write('<pre>');
    visitChildren(node);
    _str.write('</pre>');
  }
  ...
  @override
  void visitElement(HtmlElement node) {
